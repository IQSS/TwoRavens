...
get_accuracies <- function(metadata, n, Beta){
	...
	for(i in 1:k){
		...
		# column in metadata that contains either mean, quantile, histogram, or covariance. 
		...
		if(stat == "mean"){
			range <- abs(as.numeric(metadata$UpperBound[i]) - as.numeric(metadata$LowerBound[i])) 
			new_acc <- Mean.getAccuracy(eps_i, del_i, range, n, Beta) 
		}
	
		else if(stat == "quantile"){
			granularity <- as.numeric(metadata$Granularity[i])
			range <- c(as.numeric(metadata$LowerBound[i]), as.numeric(metadata$UpperBound[i]))
			...
			for(i in 1:length(metadata[ ,1])){
				if(metadata$Covariance[i] == 1){
					upper <- c(upper, as.numeric(metadata$UpperBound[i]))
					lower <- c(lower, as.numeric(metadata$LowerBound[i]))
				}
			}
	...
	if(stat == "mean"){
		...
		for(j in 1:length(metadata[ ,1])){
			if(metadata$Covariance[j] == 1){      #if attribute is included in the covariance matrix
				upper <- c(upper, as.numeric(metadata$UpperBound[j]))	 #append values to upper and lower bound vectors
				lower <- c(lower, as.numeric(metadata$LowerBound[j]))
			}
		}
